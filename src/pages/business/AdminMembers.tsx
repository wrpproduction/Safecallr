...
import { Users, LayoutDashboard, History, Settings, Shield, ChevronRight } from "lucide-react";
    ...
    <div className="min-h-screen bg-slate-50 flex text-[#0F1B3D] font-sans">
      <aside className="w-64 bg-[#0F1B3D] shrink-0 min-h-screen text-white flex flex-col justify-between border-r border-[#1a2d5e]">
        <div>
          ...
          <nav className="p-4 space-y-1">
            ...
            <Link to="/business/admin/members" className="flex items-center gap-3 px-4 py-3 rounded-xl bg-[#3DFFA0] text-[#0F1B3D] font-bold text-sm transition-all shadow-md shadow-[#3DFFA0]/10">
              <Users size={18} />
            ...
            <Link to="/business/admin/settings" className="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-300 hover:text-white hover:bg-[#1a2d5e] font-medium text-sm transition-all">
              <Settings size={18} />
      ...
      <main className="flex-1 p-8 space-y-6">
        <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm max-w-4xl">
          <h1 className="text-2xl font-black text-[#0F1B3D] mb-2">Gestion des collaborateurs</h1>
          <p className="text-slate-500 text-sm mb-6">Cette page de gestion de vos collaborateurs (PAGE 2) sera implémentée en détail lors de la prochaine étape après votre validation du Dashboard administrateur.</p>
